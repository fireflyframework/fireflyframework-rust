...
`(application, profile, label)`. Any client that already speaks the
Spring Cloud Config protocol talks to it without modification.
...
are omitted when empty, and the label defaults to `main` when the
third path segment is absent.
...
The request path is percent-decoded **before** routing: an encoded
slash (`%2F`) separates segments, and a path containing an invalid
percent-escape is rejected with `400 Bad Request` before the handler
runs.
...
Alongside the `Store`/`MemoryStore`/`router` surface above, the crate
ships a lower-level **`config_server` backend** layer — filesystem and
Git stores exposed via `ConfigBackend`. Where `Store` answers a fully
composed `Environment` lookup, a `ConfigBackend` works one tier lower:
it reads, writes, and lists individual `ConfigSource` bundles keyed by
...
| `MemoryBackend::new()`                          | Map-backed backend for tests                                            |
...
The `Store` trait carries a default `save` method that returns
`ConfigServerError::Unsupported`. Read-only stores (including
...
applications, and JSON wire-shape compatibility — including sorted
`source` keys, `version`/`state` omission, and the trailing newline.
The backend suite (`tests/backend.rs`) exercises config-server
composition, tiered overlay resolution, and the Git backend — the Git
tests build a **local** repository in a tempdir via the system `git`
binary, so no network access is required.
