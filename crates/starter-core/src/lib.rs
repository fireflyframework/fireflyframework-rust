...
pub use firefly_actuator::{
    ActuatorConfig, HealthComposite, HealthIndicator, HealthResult, HealthStatus, IndicatorFn,
    InfoContributor, MetricRegistry,
};
...
pub use firefly_observability::{Indicator, LogConfig, LogFormat};
pub use firefly_scheduling::Scheduler;
pub use firefly_web::{CorrelationLayer, IdempotencyConfig, IdempotencyLayer, ProblemLayer};
...
impl Core {
    ...
    pub fn new(cfg: CoreConfig) -> Self {
        ...
        let log = cfg
            .log
            .unwrap_or_else(|| LogConfig::new().with_service(app_name.clone()));
    ...
    pub fn apply_middleware(&self, router: Router) -> Router {
        router.layer(
            ...
            // matching Go's Problem(Correlation(Idempotency(next))).
            ServiceBuilder::new()
                .layer(ProblemLayer::new())
                .layer(CorrelationLayer::new())
                .layer(IdempotencyLayer::new(self.idempotency.clone())),
        )
    }
