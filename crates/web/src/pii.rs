...
/// identical patterns to the Go/Java/.NET ports. Order matters: most
/// specific first.
static PII_PATTERNS: LazyLock<Vec<(&'static str, Regex)>> = LazyLock::new(|| {
    vec![
        ...
        (
            "iban",
            Regex::new(r"\b[A-Z]{2}[0-9]{2}[A-Z0-9]{10,30}\b").expect("iban regex"),
        ),
        (
            "card",
            Regex::new(r"\b(?:\d[ -]*?){12,19}\b").expect("card regex"),
        ),
        (
            "phone",
            Regex::new(r"\+[1-9][0-9]{6,14}\b").expect("phone regex"),
        ),
    ]
});
