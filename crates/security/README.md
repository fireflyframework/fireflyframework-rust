...
* **Context propagation.** The `Authentication` lives on the request's
  `http::Extensions`. Handlers read it with axum's
  `Extension<Authentication>` extractor; middleware can use the helpers
  `with_authentication` / `authentication_from` / `must_auth_from`.
* **Wire compatibility.** Rejections are RFC 7807
  ...
  `…/forbidden`), with stable `detail` strings
  (`firefly/security: unauthenticated`, `authentication required`,
  `required role missing`).
* **No nil panics.** `BearerConfig::new` takes the verifier up front so
  a missing-verifier state is unrepresentable.
* **Errors.** `SecurityError` is a `thiserror` enum with stable
  `Display` strings; verifier failures flow through
...
// Standalone symmetric (HMAC) JWT primitive.
...
## OAuth2 and JWT surface

On top of the bearer/RBAC core above, `firefly-security` ships a full
OAuth2 resource-server and authorization-server layer. Idioms are
idiomatic Rust throughout: builders rather than decorators, explicit
construction rather than DI, request extensions rather than ambient
context, and typed predicates rather than expression-language strings.
...
* **`JwtService`** — a standalone **symmetric** (HMAC) JWT primitive,
  the reusable counterpart to the RS256, verify-only `JwksVerifier`:
  HS256 default (HS384/HS512 configurable), `expiration_seconds`
  injecting an `exp` claim on `encode` when one is absent, `exp`
  **required** on `decode` (a never-expiring token is rejected), and
  `to_authentication` (`sub` → principal, `roles`/`permissions` →
  roles/authorities). It satisfies the `Verifier` port, so it drops
  straight into `BearerLayer` for symmetric-token APIs, workers, CLIs,
  and inter-service tokens without any IdP. Errors carry an
  `Invalid token: <detail>` message shape; an asymmetric algorithm is
  rejected at construction.
* **`SessionAuthenticationLayer`** — a tower layer that restores the
...
* **`FilterChain` URL DSL** — `permit_pattern`, `require_pattern`,
  `require_authority`, `authenticated`, and `deny` use fnmatch-style
  globs (`/api/admin/**`); first matching rule wins.
  **Deny-by-default (fail-closed):** once any rule is declared, a
  request matching no rule is rejected with 403. Re-open the unmatched
  tail with the catch-all `any_request_permit` /
  `any_request_authenticated` / `any_request_deny`. A chain with no
  rules at all is a no-op and passes every request through (never a
  blanket lockout).
* **`guards`** — typed `AuthorizationGuard` predicates
  ...
  …)`) composed with `and` / `or` / `not`, the typed alternative to
  expression-language authorization strings; `authorize` splits 401
  (no/anonymous principal) from 403 (predicate false).
...
* **`oauth2`** module:
  ...
  * `AuthorizationServer` — `client_credentials` + `refresh_token`
    ...
    constant-time client authentication, with the standard RFC-6749
    error codes (`INVALID_CLIENT`, `UNAUTHORIZED_CLIENT`,
  ...
  * `TokenStore` port + `InMemoryTokenStore`, `RedisTokenStore`
    ...
    table name).
* **`PasswordEncoder` + `BcryptPasswordEncoder`** — a standalone,
  reusable credential hash/verify primitive, usable independently of any
  IdP (a worker, a custom user store, a rotation job).
  `BcryptPasswordEncoder::new()` uses a default work factor
  (`DEFAULT_ROUNDS = 12`); `with_rounds(n)` sets it explicitly. `hash` /
  `verify` return a `Result<_, SecurityError>` (a malformed stored hash
  surfaces as a value, not a panic); a correct-but-mismatching password
  is `Ok(false)`. Standard `$2b$` bcrypt hashes interchange with the
  `firefly-idp-internal-db` adapter.

### OAuth2 / session tests
...
* `pyfly_parity_test.rs` — `CsrfLayer` and the `FilterChain` glob /
  `deny` / `authenticated` / `require_authority` / role-hierarchy
  behaviours through the real tower stack.
