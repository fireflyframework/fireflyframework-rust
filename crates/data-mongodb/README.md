...
It provides `MongoRepository`, `BaseDocument`, the `Audited` hook, and
(feature `actuator`) `MongoHealthIndicator`. Specification handling is via
`firefly_data::Specification::to_mongo()`, described above.
...
```toml
firefly-data-mongodb = { version = "26.6.6", features = ["actuator"] }
```
