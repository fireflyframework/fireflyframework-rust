...
`firefly-config` provides **typed, layered configuration
binding** for Rust, in the style of mature configuration frameworks.
Application authors declare a `serde`-deserializable
...
The binder is **type-driven**: `"9090"` binds onto an integer field, `"alpha,beta"`
...
`String`, `bool` (accepting `1/t/T/true/TRUE/True`,
...
this keeps the binder dependency-light.

## YAML subset

Files are parsed by a line-by-line embedded
YAML-subset scanner — no general-purpose YAML dependency — so the
flattened output is fully deterministic for any given
`application.yaml`:

- nested mappings become dot-joined lower-cased keys (each parent
  mapping key also yields an empty-string entry);
...
- aliases / anchors / multi-doc / tags / flow sequences are not
  interpreted (deliberate — bring your own parser if you need them).
...
On top of the core binding surface, the crate provides a full
configuration subsystem — placeholder resolution, relaxed keys, runtime
reload, property-source introspection, multi-profile support, profile
expressions, in-process application events, and a remote config client.
...
carrying the changed top-level keys.
...
expression evaluates to `false` (never panics). The active list is taken
as a slice so it composes directly with `active_profiles`.
...
and yield an empty map (soft-miss behavior); transport/decode
...
| `mask::{mask_value, is_sensitive_key, sanitize_uri, MASK}` | Secret masking for property values |
...
| `RefreshScopeRefreshedEvent` | Published by `reload_and_publish` after a refresh |
| `ReloadableConfig::reload_and_publish(&bus)` | Reload + publish `RefreshScopeRefreshedEvent` |
...
The suite covers the core binding contract (static + YAML + env merge
order, profile selection, optional-YAML absence tolerance, flag
precedence, duration via `i64`) plus leaf-kind coverage for every
integer width and float, the full `ParseBool` acceptance set,
`Option`/enum/map binding, `serde_json::Value` binding through
`deserialize_any`, YAML flattening edge cases, `Send + Sync` bounds, and
`load` inside a tokio task.

The advanced-configuration layer is exercised end to end: placeholder
env/config/default/recursion cases, kebab↔snake binding,
reload-on-file-change with changed-key reporting, ordered+masked
property sources, multi-profile overlays, and `ConfigClient` against an
in-process axum mock (flattening precedence, basic auth, soft-miss on
non-2xx, transport-error fallback).
